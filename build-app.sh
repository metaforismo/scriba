...
create_dmg() {
    ...
    if [ -z "${VITE_SCRIBA_ENV}" ]; then
      export VITE_SCRIBA_ENV="${SCRIBA_ENV:-dev}"
      print_info "Set VITE_SCRIBA_ENV=${VITE_SCRIBA_ENV} for build-time embedding"
    fi
    bun run electron-vite build
    ...
    if [ -d "dist" ]; then
        ...
        local dmg_name
        if [ "$stage" = "prod" ]; then
          dmg_name="Scriba-Installer.dmg"
        else
          dmg_name="Scriba-${stage}-Installer.dmg"
        fi
...
create_windows_installer() {
    ...
    docker run --rm --platform linux/amd64 \
      ...
      --env VITE_SCRIBA_VERSION="${VITE_SCRIBA_VERSION}" \
      --env SCRIBA_ENV="${SCRIBA_ENV}" \
      ...
      bash -c "
        ...
        exe_path=\$(ls -t dist/Scriba*.exe 2>/dev/null | head -n 1)
        if [ -n \"\$exe_path\" ]; then
          dest_name=\$([ \"\${SCRIBA_ENV:-dev}\" = \"prod\" ] && echo \"Scriba-Installer.exe\" || echo \"Scriba-\${SCRIBA_ENV}-Installer.exe\")
...
main() {
    ...
    print_status "Starting Scriba $PLATFORM build process..."
    echo
    ...
    case $PLATFORM in
        "mac")
            ...
            if [ -z "${SCRIBA_ENV}" ] || [ "${SCRIBA_ENV}" = "prod" ]; then
              print_info "Your DMG installer is ready: dist/Scriba-Installer.dmg"
            else
              print_info "Your DMG installer is ready: dist/Scriba-${SCRIBA_ENV}-Installer.dmg"
            fi
            ;;
        "windows")
            create_windows_installer
