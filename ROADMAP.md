...
### Iteration 63 — 2026-06-11 (multi-reviewer quality pass: perf, Windows, hands-free, iOS) — PRs #27, #28
- **Process:** user-requested "continue" → ran 3 parallel review agents over the merged codebase (performance hot path, Windows compatibility, recent-feature correctness), verified each finding against the code, then fixed in two batches (desktop direct, iOS via a fixer agent in a worktree).
- **Perf (PR #27):** text-writer no longer blocks ~1s per dictation on clipboard restore (restore moved to the Electron main process; the comment in the Rust code claimed a background thread that didn't exist); global key events now forwarded only to subscribed webContents (was: every OS keystroke broadcast to all windows — perf + privacy); interactions list no longer ships `raw_audio` BLOBs on every refresh (summary query + lazy fetch by id); volume meter throttled to ~15Hz at the source; `awaitDrainComplete` no longer orphans the first caller / leaks listeners on overlapping stops.
- **Hands-free (PR #27):** toggling the setting off mid-session no longer orphans the recording; stop→start now serialized in `scribaSessionManager` (a pending tap interrupted by a new shortcut could interleave `initialize()` with the old session's teardown); accidental double-tap on the stop tap swallowed (refractory window); global-disable now cancels instead of completing (no surprise text insertion). +7 regression tests.
- **Windows (PR #27):** Ctrl+V/Ctrl+C sent as raw VK codes (Key::Unicode silently failed on non-Latin layouts — core paste broken); `Unknown(179)` fn-normalization gated to macOS (it's media Play/Pause on Windows); mic privacy-denial now detected via `getMediaAccessStatus` (was hardcoded `true`); local GNU cross-compile now mirrors `.exe`s into the MSVC dir electron-builder expects (local `bun build:app:windows` was broken); dashboard close hides to tray instead of quitting the app mid-dictation; macOS-only mute toggle hidden elsewhere. Verified with `cargo check --target x86_64-pc-windows-gnu`.
- **iOS (PR #28):** `finishRecording` made idempotent + recorder samples cleared on stop (double-stop could transcribe and insert the same audio twice — reachable via double tap or AirPods removal firing two notifications); keyboard dismissal mid-dictation now cancels (mic stayed hot, late transcript could land in the wrong field); stale `SFSpeechRecognizer` results ignored after stop; level metering throttled ~15Hz with the waveform isolated so the whole keyboard stops re-rendering 25–50×/s. +5 unit tests (36 green on simulator).
- **Env:** local Rust stable was 1.83 (couldn't build the edition-2024 crate) → updated to 1.96; disk was 100% full (171MB free) → cleared rebuildable caches (Xcode DerivedData 8.7GB, stale toolchain, cargo download cache) → 14GB free.
- **Note:** repo-wide `bun type-check` has 61 pre-existing errors on main (unchanged by this pass — verified by diffing against main's baseline); tests are the green gate.
- **Next:** remaining review findings not yet applied: text-writer as a persistent daemon (spawn cost ~20-80ms/dictation), sync echo (audio re-downloaded after upload + unpaginated first sync), server `enhancePcm16` blocking the event loop, iOS base64 upload overhead. Plus the user's open product decisions (streaming ASR provider, Android, pricing).
