...
import { scribaHttpClient } from '../clients/scribaHttpClient'
...
export function registerIPC() {
  ...
  handleIPC('start-trial-after-onboarding', async () => {
    const result = await scribaHttpClient.post('/trial/start', undefined, {
      requireAuth: true,
    })
  ...
  handleIPC('auth0-send-verification', async (_e, { dbUserId }) => {
    ...
    return scribaHttpClient.post('/auth0/send-verification', {
      dbUserId,
  ...
  handleIPC('trial:complete', async () => {
    return scribaHttpClient.post('/trial/complete')
  })

  // Billing routes proxy
  handleIPC('billing:create-checkout-session', async () => {
    return scribaHttpClient.post('/billing/checkout')
  })
  ...
  handleIPC('billing:status', async () => {
    return scribaHttpClient.get('/billing/status')
  })

  handleIPC('billing:cancel-subscription', async () => {
    return scribaHttpClient.post('/billing/cancel')
  })

  handleIPC('billing:reactivate-subscription', async () => {
    return scribaHttpClient.post('/billing/reactivate')
  })
    ...
    try {
      ...
      if (response.ok) {
        ...
        const isValidResponse = text.includes(
          'Welcome to the Scriba Connect RPC server!',
        )
  ...
  handleIPC('analytics:resolve-install-token', async () => {
    return scribaHttpClient.get('/link/resolve')
  })
