...
mock.module('@/app/generated/scriba_pb', () => ({
  ScribaService: { typeName: 'ScribaService' },
...
describe('GrpcClient Business Logic Tests', () => {
  ...
  describe('Transcription Stream Business Logic', () => {
    ...
    test('should include metadata in transcription', async () => {
      ...
      await grpcClient.transcribeStream(audioStream, ScribaMode.TRANSCRIBE)
    ...
    test('should handle transcription errors gracefully', async () => {
      ...
      expect(
        grpcClient.transcribeStream(audioStream, ScribaMode.TRANSCRIBE),
