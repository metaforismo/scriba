...
class GrpcClient {
  private client: ReturnType<typeof createClient<typeof ScribaService>>
  ...
  constructor() {
    ...
    this.client = createClient(ScribaService, transport)
  ...
  private async getHeadersWithMetadata(mode: ScribaMode) {
  ...
  async transcribeStream(stream: AsyncIterable<AudioChunk>, mode: ScribaMode) {
