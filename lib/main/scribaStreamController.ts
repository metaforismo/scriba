...
export class ScribaStreamController {
  ...
  public async initialize(mode: ScribaMode): Promise<boolean> {
    ...
    console.log('[ScribaStreamController] Starting new interaction stream.')
  ...
  public async startGrpcStream(): Promise<{
    ...
    if (this.hasStartedGrpc) {
      log.warn('[ScribaStreamController] gRPC stream already started')
      throw new Error('Stream already started')
    }

    console.log('[ScribaStreamController] Starting gRPC stream immediately')
    this.hasStartedGrpc = true
    this.abortController = new AbortController()
    const abortSignal = this.abortController.signal
    const timingEventName =
      this.currentMode === ScribaMode.EDIT
  ...
  public setMode(mode: ScribaMode) {
    if (!this.audioStreamManager.isCurrentlyStreaming()) {
      log.warn('[ScribaStreamController] Cannot change mode - no active stream')
      return
    }

    this.currentMode = mode
    console.log(`[ScribaStreamController] Mode changed to ${mode}`)
  ...
  public scheduleConfigUpdate(context: ContextData) {
    if (!this.audioStreamManager.isCurrentlyStreaming()) {
      log.warn('[ScribaStreamController] Cannot send config - no active stream')
      return
    }

    console.log('[ScribaStreamController] Queueing config update')
    const config = this.buildStreamConfig(context)
    this.configQueue.push(config)
  }

  private sendModeUpdate(mode: ScribaMode) {
    console.log(`[ScribaStreamController] Sending mode update: ${mode}`)
  ...
  public endInteraction() {
    if (!this.audioStreamManager.isCurrentlyStreaming()) {
      log.warn('[ScribaStreamController] No active stream to end')
      return
    }

    console.log('[ScribaStreamController] Ending interaction stream')
    this.stopStreaming()
  }

  public cancelTranscription() {
    if (!this.audioStreamManager.isCurrentlyStreaming()) {
      log.warn('[ScribaStreamController] No active stream to cancel')
      return
    }

    console.log('[ScribaStreamController] Cancelling transcription')
  ...
  private async *createStreamGenerator(): AsyncGenerator<TranscribeStreamRequest> {
    console.log(
      '[ScribaStreamController] Starting stream generator (audio-first mode)',
    )
    ...
    for await (const audioChunk of this.audioStreamManager.streamAudioChunks()) {
      if (this.isCancelled) {
        console.log(
          '[ScribaStreamController] Stream cancelled, stopping generator',
        )
        break
      }
      ...
      while (this.configQueue.length > 0) {
        ...
        console.log('[ScribaStreamController] Sending config update from queue')
        yield configMessage
      }
    ...
    while (this.configQueue.length > 0) {
      const configMessage = this.configQueue.shift()!
      console.log(
        '[ScribaStreamController] Sending final config update from queue',
      )
      yield configMessage
    }
  }
...
export const scribaStreamController = new ScribaStreamController()
