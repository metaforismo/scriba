...
import { scribaSessionManager } from './scribaSessionManager'
import { initializeAutoUpdater } from './autoUpdaterWrapper'
import { teardown } from './teardown'
import { SCRIBA_ENV } from './env'
...
app.whenReady().then(async () => {
  ...
  const appId = SCRIBA_ENV === 'prod' ? 'ai.scriba.scriba' : `ai.scriba.scriba-${SCRIBA_ENV}`
  registerResourcesProtocol()
