...
const timingAnalyticsTemplate = {
  index_patterns: ['scriba-timing-analytics-*'],
  template: {
    settings: { index: templateSettings },
    mappings: {
      dynamic: false,
...
export const handler = async (_event: Event) => {
  // Templates
  await request('/_index_template/scriba-client-logs', 'PUT', clientTemplate)
  await request('/_index_template/scriba-server-logs', 'PUT', serverTemplate)
  await request(
    '/_index_template/scriba-timing-analytics',
    'PUT',
  ...
  try {
    await request(
      '/_plugins/_ism/policies/scriba-retain-forever',
      'PUT',
      ismPolicy,
    )
  } catch (e: any) {
  ...
  const addPolicy = (t: any) => ({
    ...
    template: {
      ...
      settings: {
        ...t.template.settings,
        'index.plugins.index_state_management.policy_id': 'scriba-retain-forever',
      },
    },
  })
  await request(
    '/_index_template/scriba-client-logs',
    'PUT',
    addPolicy(clientTemplate),
  )
  await request(
    '/_index_template/scriba-server-logs',
    'PUT',
