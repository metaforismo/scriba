...
export function createFargateTask(
  ...
  const fargateTaskRole = new IamRole(scope, 'ScribaFargateTaskRole', {
  ...
  const taskExecutionRole = new IamRole(scope, 'ScribaTaskExecRole', {
    assumedBy: new ServicePrincipal('ecs-tasks.amazonaws.com'),
    managedPolicies: [
  ...
  const taskDefinition = new FargateTaskDefinition(scope, 'ScribaTaskDefinition', {
  ...
  const containerName = 'ScribaServerContainer'
  ...
  taskDefinition.addContainer(containerName, {
    ...
    environment: {
      ...
      SCRIBA_ENV: config.stageName,
      SHOW_ALL_REQUEST_LOGS: 'true',
    },
    logging: new AwsLogDriver({
      streamPrefix: 'scriba-server',
      logGroup: config.serverLogGroup,
    }),
  })

  const taskLogsPolicy = new Policy(scope, 'ScribaTaskLogsPolicy', {
    statements: [
      new PolicyStatement({
        actions: [
          'logs:CreateLogGroup',
        ...
        resources: [
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/scriba/${config.stageName}/client`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/scriba/${config.stageName}/client:log-stream:*`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/scriba/${config.stageName}/server`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/scriba/${config.stageName}/server:log-stream:*`,
        ],
      }),
    ],
  })
