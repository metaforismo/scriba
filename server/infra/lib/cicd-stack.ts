...
export class GitHubOidcStack extends Stack {
  constructor(scope: Construct, id: string, props: GitHubOidcStackProps) {
    ...
    const principal = new OpenIdConnectPrincipal(oidc, {
      StringLike: {
        'token.actions.githubusercontent.com:sub': `repo:heyito/scriba:*`,
        'token.actions.githubusercontent.com:aud': 'sts.amazonaws.com',
      },
    })

    // ─── create the CI/CD role ─────────────────────────────────────────────────
    const ciCdRole = new Role(this, 'ScribaGitHubCiCdRole', {
      assumedBy: principal,
      roleName: 'ScribaGitHubCiCdRole',
      description: 'GitHub Actions can assume this via OIDC',
    })

    // ─── create s3 bucket for releases ────────────────────────
    props.stages.forEach(stage => {
      const bucketName = `${stage}-${SCRIBA_PREFIX.toLowerCase()}-releases`
      const bucket = new Bucket(this, `${stage}-ScribaReleasesBucket`, {
        bucketName,
    ...
    // ─── CloudFormation on any of our “Scriba*” stacks ──────────────────────────────
    const cfnArnPattern = `arn:aws:cloudformation:${this.region}:${this.account}:stack/${SCRIBA_PREFIX}*/*`
    ciCdRole.addToPolicy(
      new PolicyStatement({
        effect: Effect.ALLOW,
        actions: [
