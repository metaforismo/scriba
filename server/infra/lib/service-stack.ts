...
export class ServiceStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: ServiceStackProps) {
    ...
    const groqApiKeySecret = Secret.fromSecretNameV2(
      this,
      'GroqApiKey',
      `${stageName}/scriba/groq-api-key`,
    )

    const cerebrasApiKeySecret = Secret.fromSecretNameV2(
      this,
      'CerebrasApiKey',
      `${stageName}/scriba/cerebras-api-key`,
    )

    const stripeSecretKeySecret = Secret.fromSecretNameV2(
      this,
      'StripeSecretKey',
      `${stageName}/scriba/stripe-secret-key`,
    )

    const stripeWebhookSecret = Secret.fromSecretNameV2(
      this,
      'StripeWebhookSecret',
      `${stageName}/scriba/stripe-webhook`,
    )

    // Setup domain and certificate
    const zone = HostedZone.fromLookup(this, 'HostedZone', {
      domainName: 'scriba-api.com',
    })

    const domainName = `${stageName}.scriba-api.com`
    ...
    // Create ECS cluster
    const cluster = new Cluster(this, 'ScribaEcsCluster', {
      vpc: props.vpc,
      clusterName: `${stageName}-${CLUSTER_NAME}`,
    })

    // Create S3 buckets
    const logBucket = new Bucket(this, 'ScribaAlbLogsBucket', {
      bucketName: `${stageName}-${this.account}-${this.region}-scriba-alb-logs`,
    ...
    const firehoseBackupBucket = new Bucket(this, 'ScribaFirehoseBackupBucket', {
      bucketName: `${stageName}-${this.account}-${this.region}-scriba-firehose-bucket`,
      removalPolicy: isDev(stageName)
        ? RemovalPolicy.DESTROY
        : RemovalPolicy.RETAIN,
      blockPublicAccess: BlockPublicAccess.BLOCK_ALL,
    ...
    scalableTarget.scaleOnCpuUtilization('ScribaServerCpuScalingPolicy', {
    ...
    alb.logAccessLogs(logBucket, 'scriba-alb-access-logs')
    ...
    const firehoseRole = IamRole.fromRoleArn(
      this,
      'ScribaFirehoseRoleImported',
      `arn:aws:iam::${this.account}:role/${stageName}-ScribaFirehoseRole`,
      { mutable: true },
    )
