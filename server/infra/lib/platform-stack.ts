...
export class PlatformStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: PlatformStackProps) {
    ...
    const dbSecurityGroup = new SecurityGroup(this, 'ScribaDbSecurityGroup', {
      vpc: props.vpc,
      description: 'Allow ECS Fargate service to connect to Aurora',
      allowAllOutbound: true,
    })

    const dbCredentialsSecret = new Secret(this, 'ScribaDbCredentials', {
      secretName: `${stageName}/scriba-db/dbadmin`,
    ...
    const dbCluster = new DatabaseCluster(this, 'ScribaAuroraServerless', {
    ...
    this.serviceRepo = new Repository(this, 'ScribaServiceRepo', {
    ...
    this.blobStorageBucket = new Bucket(this, 'ScribaBlobStorage', {
      bucketName: `${stageName}-${this.account}-${this.region}-scriba-blob-storage`,
      removalPolicy: isDev(stageName)
        ? RemovalPolicy.DESTROY
        : RemovalPolicy.RETAIN,
    ...
    const firehoseRole = new Role(this, 'ScribaFirehoseRole', {
      assumedBy: new ServicePrincipal('firehose.amazonaws.com'),
      roleName: `${stageName}-ScribaFirehoseRole`,
    })

    // Cognito resources for OpenSearch Dashboards authentication
    const userPool = new UserPool(this, 'ScribaOsUserPool', {
      userPoolName: `${stageName}-scriba-os-userpool`,
      selfSignUpEnabled: true,
      signInAliases: { email: true, username: true },
      accountRecovery: undefined,
    })
    new UserPoolClient(this, 'ScribaOsUserPoolClient', {
      userPool,
      userPoolClientName: `${stageName}-scriba-os-client`,
      generateSecret: false,
      authFlows: { userSrp: true, userPassword: true },
    })
    new UserPoolDomain(this, 'ScribaOsUserPoolDomain', {
      userPool,
      cognitoDomain: {
        domainPrefix: `${stageName}-${this.account}-scriba-os`.toLowerCase(),
      },
    })
    const identityPool = new CfnIdentityPool(this, 'ScribaOsIdentityPool', {
      allowUnauthenticatedIdentities: true,
      identityPoolName: `${stageName}-scriba-os-identitypool`,
      // Leave providers empty so OpenSearch can register its own App Client
    })
    const authenticatedRole = new Role(this, 'ScribaOsCognitoAuthenticatedRole', {
    ...
    const unauthenticatedRole = new Role(
      this,
      'ScribaOsCognitoUnauthenticatedRole',
      {
        assumedBy: new FederatedPrincipal(
          'cognito-identity.amazonaws.com',
          {
    ...
    new CfnIdentityPoolRoleAttachment(this, 'ScribaOsIdentityPoolRoleAttachment', {
    ...
    const cognitoAccessRole = new Role(this, 'ScribaOsCognitoAccessRole', {
    ...
    const domain = new Domain(this, 'ScribaLogsDomain', {
      domainName: `${stageName}-scriba-logs`,
    ...
    domain.addAccessPolicies(
      new PolicyStatement({
        ...
        conditions: {
          ...
          ArnLike: {
            'aws:SourceArn': [
              `arn:aws:firehose:${this.region}:${this.account}:deliverystream/${stageName}-scriba-client-logs`,
              `arn:aws:firehose:${this.region}:${this.account}:deliverystream/${stageName}-scriba-server-logs`,
            ],
          },
        },
      }),
    ...
    Tags.of(this).add('Project', 'Scriba')
  }
}
