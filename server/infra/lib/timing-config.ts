...
export function createTimingInfrastructure(
  ...
  const timingBucket = new Bucket(scope, 'ScribaTimingDataBucket', {
    bucketName: `${config.stageName}-${config.accountId}-${config.region}-scriba-timing-data`,
  ...
  const timingDLQ = new Queue(scope, 'ScribaTimingDLQ', {
    queueName: `${config.stageName}-scriba-timing-dlq`,
    retentionPeriod: Duration.days(14), // Keep failed messages for 14 days
    removalPolicy: isDev ? RemovalPolicy.DESTROY : RemovalPolicy.RETAIN,
  })

  // Create main queue for timing events with DLQ
  const timingQueue = new Queue(scope, 'ScribaTimingQueue', {
    queueName: `${config.stageName}-scriba-timing-queue`,
  ...
  const timingMergerLambda = new NodejsFunction(scope, 'ScribaTimingMerger', {
