...
The Scriba transcription server provides gRPC-based speech-to-text services for the Scriba voice assistant application. This server handles audio transcription, user data management, and API authentication.
...
BLOB_STORAGE_BUCKET=scriba-audio-storage
...
- Auto-creates the `scriba-audio-storage` bucket
...
- **Auto-bucket creation**: `scriba-audio-storage` created automatically
...
# Run migrations
docker compose exec scriba-grpc-server bun run db:migrate
```
...
The API is defined in `src/scriba.proto`. Key services:

```protobuf
service ScribaService {
  // Transcription
...
The Scriba desktop app automatically connects to `localhost:3000`. Ensure the server is running before starting the desktop application.
