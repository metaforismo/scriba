...
function renderDeepLinkHtml(targetUrl: string): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Returning to Scriba…</title></head><body>
  <p>If you are not redirected automatically, click below:</p>
  <a href="${escaped}">Return to Scriba</a>
...
export const registerBillingPublicRoutes = async (fastify: FastifyInstance) => {
  const APP_PROTOCOL = getEnv('APP_PROTOCOL') // e.g., scriba-dev or scriba
