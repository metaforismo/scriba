...
describe('registerBillingPublicRoutes', () => {
  ...
  it('renders success page with session_id', async () => {
    ...
    expect(res.body).toContain(
      'scriba-dev://billing/success?session_id=cs_test_123',
    )
    expect(res.body).toContain('Returning to Scriba')
    await app.close()
  })
  ...
  it('renders success page without session_id', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(res.headers['content-type']).toContain('text/html')
    expect(res.body).toContain('scriba-dev://billing/success')
  ...
  it('renders cancel page', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(res.headers['content-type']).toContain('text/html')
    expect(res.body).toContain('scriba-dev://billing/cancel')
    expect(res.body).toContain('Returning to Scriba')
    await app.close()
  })
  ...
  it('escapes quotes in deeplink URL', async () => {
    ...
    process.env.APP_PROTOCOL = 'scriba-dev'
