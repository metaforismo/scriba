...
// The request body carries the audio base64-encoded (≈4/3 the byte size) inside
// a small JSON envelope. Without an explicit per-route limit Fastify's 1 MB
// default would reject any dictation longer than ~20 s, so size the limit to the
// audio cap plus base64 + JSON overhead.
const MAX_REQUEST_BODY_BYTES = Math.ceil(MAX_AUDIO_BYTES * (4 / 3)) + 1024
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const transcript = await cleanupTranscript(
          rawTranscript,
          cleanupLevel,
          {
        ...
        if (error instanceof ClientError) {
          reply
            .code(422)
            .send({ error: 'Transcription failed', code: error.code })
          return
        }
        reply
          .code(502)
