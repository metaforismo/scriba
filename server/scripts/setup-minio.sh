...
if ! command -v mc &> /dev/null; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        echo "  docker run --rm -it --entrypoint=/bin/sh minio/mc -c \"mc alias set local http://host.docker.internal:9000 minioadmin minioadmin && mc mb local/scriba-audio-storage\""
        exit 1
    fi
fi
...
BLOB_BUCKET=${BLOB_STORAGE_BUCKET:-scriba-blob-storage}
...
TIMING_BUCKET=${TIMING_BUCKET:-scriba-timing-storage}
